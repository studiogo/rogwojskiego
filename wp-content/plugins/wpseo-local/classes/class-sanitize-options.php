...
if ( ! class_exists( 'WPSEO_Local_Sanitize_Options' ) ) {
	...
	class WPSEO_Local_Sanitize_Options {
		...
		public function sanitize_options( $new_value ) {
			$toggle_values = array( 'on', 'off' );

			$new_value['use_multiple_locations'] = isset( $new_value['use_multiple_locations'] ) && in_array( $new_value['use_multiple_locations'], $toggle_values, true ) ? sanitize_title( $new_value['use_multiple_locations'] ) : WPSEO_Local_Core::$defaults['use_multiple_locations'];
			...
			$new_value['locations_taxo_slug'] = isset( $new_value['locations_taxo_slug'] ) ? sanitize_title( $new_value['locations_taxo_slug'] ) : WPSEO_Local_Core::$defaults['locations_taxo_slug'];
			...
			// Opening hours.
			$new_value['hide_opening_hours'] = isset( $new_value['hide_opening_hours'] ) && in_array( $new_value['hide_opening_hours'], $toggle_values, true ) ? sanitize_title( $new_value['hide_opening_hours'] ) : WPSEO_Local_Core::$defaults['hide_opening_hours'];
			...
			$new_value['opening_hours_24h']  = isset( $new_value['opening_hours_24h'] ) && in_array( $new_value['opening_hours_24h'], $toggle_values, true ) ? sanitize_title( $new_value['opening_hours_24h'] ) : WPSEO_Local_Core::$defaults['opening_hours_24h'];
			...
			$new_value['enhanced_search'] = isset( $new_value['enhanced_search'] ) && in_array( $new_value['enhanced_search'], $toggle_values, true ) ? sanitize_title( $new_value['enhanced_search'] ) : WPSEO_Local_Core::$defaults['enhanced_search'];
