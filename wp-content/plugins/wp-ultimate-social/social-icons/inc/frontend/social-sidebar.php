<?php
global $wpdb;
$social_sidebar = get_option('aps_social_sidebar');
$si_id = $social_sidebar['icon_set_id'];
...
if (!empty($icon_sets)) {
    ...
    $animation = $social_sidebar['animation'];
    $icon_margin = esc_attr($social_sidebar['icon_margin']);
    $sidebar_position = $social_sidebar['social_sidebar_position'];
    $half_hidden = $social_sidebar['half_hidden'];
    $half_hidden_class = ($half_hidden=='yes')?'aps-icon-sidebar-hidden':'';
    $attachment = ($social_sidebar['social_sidebar_attachment'] == 'fixed') ? 'aps-icon-sidebar-fixed' : '';
    ?>
    <div class="aps-icon-sidebar <?php echo $half_hidden_class.' '.$sidebar_position.' '.$attachment;?>">
        <?php
        $icon_counter = 0;
        ...
        if ($icon['link'] != '') {
            ...
            $icon_counter++;
                ...
                ?>
                <div class="aps-each-icon <?php echo $icon_main_class; ?>" style='margin-bottom:<?php echo str_replace('px', '', $icon_margin); ?>px;' data-aps-tooltip='<?php echo $tooltip_text ?>' data-aps-tooltip-enabled="<?php echo $icon_set->icon_tooltip; ?>" data-aps-tooltip-bg="<?php echo $tooltip_bg = ($icon_set->tooltip_background == '') ? '#000' : $icon_set->tooltip_background; ?>" data-aps-tooltip-color="<?php echo ($icon_set->tooltip_text_color == '') ? '#fff' : $icon_set->tooltip_text_color; ?>">
                    <a href="<?php echo $icon['link'] ?>" <?php echo $link_target; ?>  class="aps-icon-link animated <?php echo ($icon_set->icon_tooltip == 1) ? 'aps-tooltip' : ''; ?>" data-animation-class="<?php echo $animation; ?>">
                        <?php
