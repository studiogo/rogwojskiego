...
class WPSEO_Local_Admin_Opening_Hours {
	...
	public function opening_hours() {
		...
		foreach ( $this->wpseo_local_core->days as $key => $day ) {
			...
			echo '</div>';
			echo '<label class="wpseo_open_24h" for="' . $field_name . '_24h"><input type="checkbox" name="wpseo_local[' . $field_name . '_24h]" id="' . $field_name . '_24h" ' . checked( $value_24h, 'on', false ) . ' /> ' . __( 'Open 24 hours', 'yoast-local-seo' ) . '</label>';
