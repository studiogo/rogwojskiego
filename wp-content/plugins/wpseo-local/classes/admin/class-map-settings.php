	...
	class WPSEO_Local_Admin_Map_Settings {
		...
		public function maps_settings() {
			...
			if ( ! empty( $api_key_browser ) || ! empty( $api_key ) ) {
				...
				echo '<br>';
				echo '<a href="javascript:" class="remove_custom_image wpseo-local-hide-button' . ( ( false === $show_marker ) ? ' hidden' : '' ) . '" data-id="custom_marker">' . esc_html__( 'Remove marker', 'yoast-local-seo' ) . '</a>';
				echo '</div>';
				echo '</div>';
				if ( true == $show_marker ) {
